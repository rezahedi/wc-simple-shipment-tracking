...
// 		error_log('__construct() called!');
		...
		$this->title = __( 'Shipped Email', 'woocommerce' );
		// Description field in WooCommerce email settings
		$this->description = __( 'Shipped email is sent when an order processed for the customer who placed the order.', 'woocommerce' );
		// Default heading and subject lines in WooCommerce email settings
		$this->subject = apply_filters( 'rz_shipped_email_default_subject', __( 'Your order has been shipped.', 'woocommerce' ) );
		$this->heading = apply_filters( 'rz_shipped_email_default_heading', __( 'Your Order Has Been Shipped', 'woocommerce' ) );
		...
		$this->template_plain = 'emails/plain/simple-shipped-emailrz-simple-shipment-tracking-email.php';
	...
	// 	$email_actions[] = 'woocommerce_order_status_wc-shippeddd';
	// 	return $email_actions;
	// }

	public function myfunc_status_custom_notification( $order_id, $from_status, $to_status, $order ) {
		error_log('class :: shippeddd: '.$order->has_status( 'shippeddd' ));
